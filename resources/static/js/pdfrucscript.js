...
//Relleno de datos
	...
	async function cargarDatosVenta(ventaid) 
	{
		try 
		{
			const response = await fetch(`http://localhost:8081/ventas/factura/${ventaid}`);
			if (!response.ok) 
			{
		    ...
		    document.getElementById("nombre").value = ventaDetalles.rucnombre;
...
const cargarDatosRegistroVenta = async (ventaid) => 
{
	try 
    {
      	const response = await fetch(`http://localhost:8081/registroventa/factura/${ventaid}`);
