...
//Cerrar Sesion
	let sesionElement = document.getElementById("sesion");
			...
			buttons: [     
				{
					...
					filename: 'Reporte Ganancia Mensual',
					title:'Producto',
		...
		const listUsers = async () => {
			try 
			{
				const response = await fetch("http://localhost:8081/reportes/datosganancia");
				...
				datos.forEach((dato) => 
				{
				    ...
				    {
				        gananciasPorAnoYMes[dato.nombreAno][dato.nombreMes] = dato.saldo;
				    } 
				    else 
				    {
				        gananciasPorAnoYMes[dato.nombreAno][dato.nombreMes] += dato.saldo;
