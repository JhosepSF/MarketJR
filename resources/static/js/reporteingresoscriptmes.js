...
//Datos	   
	let dataTable;
			...
			buttons: [     
				{
					...
					filename: 'Reporte Ingreso Mensual',
		...
		dataTable.rows().every(function () {
			dataLabels.push(this.data()[0]);
			dataValues.push(parseInt(this.data()[2]));
		});
		...
		document.getElementById("lineal").addEventListener("click", function() {
			tipograf = "line"; // Actualizar el tipo de gráfico al hacer clic en "circular"
			updateChart(); // Llamar a una función para actualizar el gráfico
		});
		...
		$('#datatable_users_filter input[type="search"]').on('keyup', function () {
			...
			filteredData.each(function (value, index) {
				filteredLabels.push(value[0]);
				filteredValues.push(parseInt(value[2]));
			});
		...
		{
			let content = ``;
			let gananciasPorAnoYMes = {};
			let meses = [];
			let anos = [];
			
			try 
			{
				const response = await fetch("http://localhost:8081/reportes/datosventa/boleta");
				const datos = await response.json();
				
				datos.forEach((dato) => 
				{
				    if (!anos.includes(dato.nombreAno)) 
				    {
				        anos.push(dato.nombreAno);
				    }
				    
				    if (!meses.includes(dato.nombreMes)) 
				    {
				        meses.push(dato.nombreMes);
				    }
				
				    if (!gananciasPorAnoYMes.hasOwnProperty(dato.nombreAno)) 
				    {
				        gananciasPorAnoYMes[dato.nombreAno] = {};
				    }
				
				    if (!gananciasPorAnoYMes[dato.nombreAno].hasOwnProperty(dato.nombreMes)) 
				    {
				        gananciasPorAnoYMes[dato.nombreAno][dato.nombreMes] = dato.monto;
				    } 
				    else 
				    {
				        gananciasPorAnoYMes[dato.nombreAno][dato.nombreMes] += dato.monto;
			...
			try 
			{
				const response = await fetch("http://localhost:8081/reportes/datosventa/factura");
				const datos = await response.json();
			...
			for(let i = 0; i < anos.length; i++)
			{
				for(let e = 0; e < meses.length; e++)
				{
					content += `
					<tr>
						<td class="centered">${meses[e]}</td>
						<td class="centered">${anos[i]}</td>
						<td class="centered">${gananciasPorAnoYMes[anos[i]][meses[e]]}</td>
						<td class="centered">valor1</td>
						<td class="centered">valor1</td>
						<td class="centered">valor1</td>
						<td class="centered">valor1</td>
					</tr>`;
