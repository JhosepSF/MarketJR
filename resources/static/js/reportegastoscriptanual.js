		...
		const listUsers = async () => {
			try {
				...
				datos.forEach((dato) => 
				{
				    ...
				    if (!comprasPorAno.hasOwnProperty(dato.nombreAno)) 
				    {
				        comprasPorAno[dato.nombreAno] = dato.monto;
				    } 
				    else 
				    {
				        comprasPorAno[dato.nombreAno] += dato.monto;
				...
				for(let i = 0; i < anos.length; i++)
				{
					content += `
						<tr>
							...
							<td class="centered">${comprasPorAno[anos[i]]}</td>
