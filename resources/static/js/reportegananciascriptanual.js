			...
			buttons: [     
				{
					...
					filename: 'Reporte Ganancia Anual',
		...
		const listUsers = async () => {
			try
			{
				...
				datos.forEach((dato) => 
				{
				    ...
				    if (!gananciasPorAno.hasOwnProperty(dato.nombreAno)) 
				    {
				        gananciasPorAno[dato.nombreAno] = dato.saldo;
				    } 
				    else 
				    {
				        gananciasPorAno[dato.nombreAno] += dato.saldo;
				...
				for(let i = 0; i < anos.length; i++)
				{
					content += `
						<tr>
							...
							<td class="centered">${gananciasPorAno[anos[i]]}</td>
