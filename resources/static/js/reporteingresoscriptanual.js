...
//Datos	   
		...
		buttons: [     
			{
				...
				filename: 'Reporte Ingreso Anual',
				title:'Ingreso Anual',
	...
	dataTable.rows().every(function () {
		...
		dataValues.push(parseInt(this.data()[1]));
	});
	...
	$('#datatable_users_filter input[type="search"]').on('keyup', function () {
		...
		filteredData.each(function (value, index) {
			...
			filteredValues.push(parseInt(value[1]));
		});
	...
	const listUsers = async () => 
	{
		...
		try 
		{
			...
			datos.forEach((dato) => 
			{
			    ...
			    if (!ventasPorAno.hasOwnProperty(dato.nombreAno)) 
			    {
			        ventasPorAno[dato.nombreAno] = dato.monto;
			    } 
			    else 
			    {
			        ventasPorAno[dato.nombreAno] += dato.monto;
		...
		for(let i = 0; i < anos.length; i++)
		{
			content += `
				<tr>
					...
					<td class="centered">${ventasPorAno[anos[i]]}</td>
