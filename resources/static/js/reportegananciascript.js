...
//Cerrar Sesion
	let sesionElement = document.getElementById("sesion");
			...
			buttons: [     
				{
					...
					filename: 'Reporte Ganacia Diaria',
		...
		dataTable.rows().every(function () {
			...
			dataValues.push(parseInt(this.data()[3]));
		});
		...
		$('#datatable_users_filter input[type="search"]').on('keyup', function () {
			...
			filteredData.each(function (value, index) {
				...
				filteredValues.push(parseInt(value[3]));
			});
		...
		const listUsers = async () => {
			try {
				...
				datos.forEach((dato) =>
				{
					...
					content += `
						<tr>
							<td class="centered">${dato.fecha}</td>
							<td class="centered">${dato.montoapertura}</td>
							<td class="centered">${dato.montocierre}</td>
							<td class="centered">${dato.saldo}</td>
							<td class="centered">${dato.deuda}</td>
							<td class="centered">${dato.estado}</td>
